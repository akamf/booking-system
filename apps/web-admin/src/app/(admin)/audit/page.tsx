...
const TABLES = [
  'bookings', 'profiles', 'guardian_links', 'user_roles',
  'blocked_times', 'opening_hours', 'activities', 'resources',
] as const;
    ...
    <div className="space-y-6">
      ...
      <form className="flex gap-3 items-end flex-wrap">
        <label className="block">
          <span className="block text-xs uppercase text-neutral-500">Table</span>
        ...
        <label className="block">
          ...
          <input type="datetime-local" name="from" defaultValue={from ?? ''} className="input" />
        </label>
        <label className="block">
          <span className="block text-xs uppercase text-neutral-500">To</span>
          <input type="datetime-local" name="to" defaultValue={to ?? ''} className="input" />
        </label>
      ...
      <div className="space-y-3">
        {rows.length === 0 ? (
          ...
          rows.map((r) => (
            <details key={r.id} className="bg-white border border-neutral-200 rounded-lg">
              <summary className="px-4 py-3 cursor-pointer text-sm flex justify-between">
                <span>
                  ...
                  <span className="font-mono text-neutral-700">{r.table_name}</span>
                  <span className="text-neutral-400"> ({r.row_id.slice(0, 8)}…)</span>
                </span>
                <time className="text-xs text-neutral-500" dateTime={r.occurred_at}>
